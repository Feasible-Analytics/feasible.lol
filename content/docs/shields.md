...
The screen shows you the address *this request* resolved to, using the same header precedence
...
- **Countries, pages and hostnames** are decided at the shard that holds your data, after the request
  ...
  `hostname_not_allowed`, and they can never appear in that header. A blocked page answers   like an accepted one; the event doesn't arrive in your reports.
...
It ships **off** rather than decided for you, because a few sites serve different content at
each, and silently merging those would be wrong.

### Preview before you save

The preview shows what saving would do and how many stored rows each merge moves. Nothing is
