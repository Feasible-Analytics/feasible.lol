...
description: "Umami has a more permissive license and a free tier. Feasible counts fewer things toward your limit."
...
Here's the difference that moves money, and it's in Umami's own words:
...
allowed to compile. Set `FEASIBLE_APP_HOSTED=false` and everything works - funnels, custom properties, the API, webhooks, MCP, raw export, teams. See
...
the public URL, so it exercises the proxy and the headers as a browser
would.
