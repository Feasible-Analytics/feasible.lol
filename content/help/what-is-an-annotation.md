...
Annotations are created in site settings or through the [API](/docs/), not from the dashboard itself — the markers on the chart are read-only. Each one is keyed to a date, and the body can run to 8 KB, so there's room for a sentence of context rather than just a label.

If you'd rather be told about a change than discover it, set up alerts instead. Spike and drop alerts are evaluated every ten minutes and go to email or a Slack webhook, capped at two per site per day so an alert never becomes noise. A drop alert is really a "has my tracking stopped" alarm, and it's the one most people should have on.
