...
**The blocking is different.** Blocklists carry different hostnames, so the two scripts don't get blocked by exactly the same people. Somewhere around [13% of visitors](https://markosaric.com/google-analytics-blocking/) block something, but that varies enormously — a developer blog and a recipe site are not the same audience, and neither is a mobile-heavy one. If it matters to you, [serving the script from your own domain](/help/can-i-proxy-the-script-from-my-own-domain/) narrows the gap.
