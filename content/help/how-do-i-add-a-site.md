...
The field people get wrong is the domain. It has to match what the browser is on. Register `example.com` and the script can run on `www.example.com`, `app.example.com` or `blog.example.com` - subdomains of the domain you registered are included, and they share visitors. Run it on `example.net` and the events are refused with the reason `unknown_site`, which the [health panel](/help/why-are-my-events-being-dropped/) will say out loud rather than dropping them quietly.
