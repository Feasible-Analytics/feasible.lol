...
- **A server-side SDK.** The IP and user agent are required arguments in every one of ours, deliberately, and each ships a helper to lift them off the incoming request. A call missing them looks exactly like a datacentre bot.
