...
description: "It's the bucket: everything that didn't carry the thing you're breaking down by. It's shown rather than dropped, on purpose."
category: "reports"
weight: 60
---
...
2. **One code path forgot it.** The signup flow sends `plan`, the checkout flow doesn't. This is the most common one, and finding it is what the bucket is for.
3. **The property is scoped to some events only** - a `search_term` on a search event and nowhere else. In which case a huge `(none)` is correct and you should filter it out rather than worry about it.
