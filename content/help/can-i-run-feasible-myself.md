...
Setting `FEASIBLE_APP_HOSTED=false` changes two categories of thing, and neither is a feature:
...
Two things to be about. Migrations never run automatically - `db migrate` is a command you type, on purpose, because a binary that migrates your database on boot will eventually do it at the worst moment. And you're the operator now: backups, TLS, upgrades and the pager are yours.
