...
server are all in your copy. The API package contains **no plan check at all** —
there's no code path where a subscription tier decides whether a request is
allowed.
...
servers", self-hosting is genuinely cheap. If it's "I'd rather not",
...
There's no CLA to sign. The product is deliberately small and we say no to
