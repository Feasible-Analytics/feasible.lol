...
**Anything else:** find the one HTML file your build tool uses as a template. There's always one, and it's the file with `<div id="root"></div>` in it.
...
Outbound link clicks, file downloads and form submissions are counted automatically, as goals named `Outbound Link: Click`, `File Download` and `Form: Submission`. Scroll depth and time on page come along too, measured only while the tab is visible and focused.
