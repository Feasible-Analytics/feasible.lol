...
Astro takes ownership of `<script>` tags it finds in components — bundling them, moving them, processing them through Vite. `is:inline` tells it to leave this one exactly as written, with its `src` and its `data-domain` intact, in the place you put it.
...
3. Check that every page actually uses that layout. Astro doesn't enforce it — a page that imports a different layout, or none, won't have the tag.
4. Build and deploy.

Content collections make this easy to get wrong. If your blog posts render through `src/layouts/BlogPost.astro` and that layout doesn't wrap the base one, your posts aren't tracked and your blog reports zero traffic.
...
Deploy, or run `astro build` and `astro preview` on a real hostname. `localhost` is deliberately not counted, so `astro dev` shows nothing however correct the tag is.
