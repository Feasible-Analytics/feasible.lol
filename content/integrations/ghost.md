...
**Per-post code injection is a separate box.** Every post and page has its own injection fields in the editor sidebar. If a tag ends up in both places, that page reports two pageviews for one visit. If your numbers look exactly doubled on one URL, check there first.
