...
There's no `defer` attribute because `next/script` handles that for you. The default `strategy` is `afterInteractive`, which is what you want. Next.js loads the script once and keeps it loaded across client-side navigations, which is exactly the behavior our route tracking depends on.
...
Next.js audiences skew technical, which means they skew blocked. If that matters to you, [serve the script from your own domain](/docs/proxying/) with a rewrite in `next.config.js` and point `data-api` at the same path. It's about six lines and it's the thing that actually works.
{{< /callout >}}
